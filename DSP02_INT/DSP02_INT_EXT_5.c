...
void Proceso() {
  if (START == 1)
  {
    while(START == 1) {}; // Esperar al falling edge para evitar falsos por la capacidad de reaccion
    contar = contar ^ 1;
  }

  if (RESET == 1)
  {
    while(RESET == 1) {}; // Esperar al falling edge para evitar falsos por la capacidad de reaccion
    contador = 1800;
    contar = 0;
    eventoContador();
  }
